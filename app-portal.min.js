/* Wed Jul 30 2025 14:50:32 GMT+0700 (Indochina Time) */"use strict";
...
app.controller("MixApplicationController", [
  ...
  function (
    ...
    $scope.themeRequest.sortBy = "id";
    $scope.themeRequest.mixDbTableName = "mixApplicationPackage";
    ...
    $scope.canDrag =
      $scope.request.sortBy !== "Priority" || $scope.request.direction !== "0";
    ...
    $scope.validate = function () {
      if ($scope.viewmodel.baseHref.indexOf("/app/") != 0) {
        $rootScope.showErrors(['baseHref must start with "/app/']);
        return false;
      }
      return true;
    };
...
app.controller("DashboardController", [
  ...
  function (
    ...
    // For rendering markdown as HTML in the template
    // Requires marked.js to be loaded globally. If not, will fallback to plain text.
    $scope.trustAsHtml = function (html) {
      if (typeof marked !== "undefined" && marked.parse) {
        return $sce.trustAsHtml(marked.parse(html || ""));
      } else {
        console.warn(
          "marked.js is not loaded. Please include marked.min.js in your HTML."
        );
        return $sce.trustAsHtml(html || "");
      }
    };
    // To enable markdown rendering, ensure you have included marked.min.js in your HTML:
    // <script src="/path/to/marked.min.js"></script>
    $scope.askAI = function () {
      $scope.connection.invoke("AskAI", $scope.message);
...
"use strict";
app.controller("GlobalSettingController", [
  "$scope",
  "$rootScope",
  "ngAppSettings",
  "$routeParams",
  ...
  function (
    ...
    $scope.updateContent = (content) => {
      $scope.viewmodel.settings = content;
    };
    $scope.getSingleSuccessCallback = function () {};
    $scope.saveSuccessCallback = function () {};
    $scope.removeCallback = function () {};
...
"use strict";
app.factory("GlobalSettingService", [
  "BaseRestService",
  "ApiService",
  "CommonService",
  function (baseService, apiService, commonService) {
    var serviceFactory = Object.create(baseService);
    serviceFactory.init("global-settings");
...
"use strict";
app.controller("MixDbDataController", [
  "$scope",
  "$rootScope",
  "ngAppSettings",
  "$routeParams",
  ...
  function (
    ...
    $scope.initRouteParams = async () => {
      $scope.requestKey = `request${$rootScope.generateKeyword(
        $location.$$path,
        "_"
      )}_${$routeParams.mixDbTableId}`;
      $scope.request = $rootScope.getRequest($scope.requestKey);
      if ($routeParams.mixDbTableName) {
        $scope.request.mixDbTableName = $routeParams.mixDbTableName;
      }
      if ($routeParams.mixDbTableId) {
        $scope.request.mixDbTableId = $routeParams.mixDbTableId;
      }
      ...
      $scope.mixDbTableId = $routeParams.mixDbTableId;
      $scope.mixDbTableName = $routeParams.mixDbTableName;
      ...
      $scope.dataUrl = `/admin/mix-db-data/list?mixDbTableId=${$scope.mixDbTableId}&mixDbTableName=${$scope.mixDbTableName}&mixDatabaseTitle=${$scope.mixDatabaseTitle}`;
      if ($routeParams.backUrl) {
        $scope.backUrl = decodeURIComponent($routeParams.backUrl);
      }
      if ($routeParams.dataContentId) {
      ...
      $scope.request.mixDbTableName = $routeParams.mixDbTableName;
      mixDbService.initDbName($scope.request.mixDbTableName);
      $scope.$apply();
    };
    $scope.loadMixDb = async () => {
      var getDatabase = await databaseService.getByName(
        $routeParams.mixDbTableName
      );
      if (getDatabase.success) {
        $scope.database = getDatabase.data;
        $scope.displayColumns = $scope.database.columns.filter(
          (c) => !$scope.hiddenColumns.includes(c.systemName)
        );
        $scope.arrOrderBy = [];
        angular.forEach($scope.database.columns, function (column) {
          $scope.arrOrderBy.push(column.systemName);
        });
        $scope.request.sortBy ??= $scope.arrOrderBy[0];
        $scope.$apply();
      }
    };
    $scope.init = async function () {
      await $scope.initRouteParams();
      $scope.isInit = true;
    };
    ...
    $scope.saveSuccess = function (data) {
      if ($scope.backUrl) {
        ...
        $location.url(
          `/admin/mix-db-data/details?dataContentId=${data}&mixDatabaseTitle=${
    ...
    $scope.edit = function (data) {
      $rootScope.goToPath(
        `/admin/mix-db-data/details?dataContentId=${data.id}&mixDbTableName=${$scope.mixDbTableName}&mixDatabaseTitle=${$scope.mixDatabaseTitle}`
      );
    };
    ...
    $scope.selectImportFile = function (files) {
      if (files !== undefined && files !== null && files.length > 0) {
        ...
        $scope.importFile.title = $scope.mixDbTableName;
        $scope.importFile.description = $scope.mixDbTableName + "'s data";
      ...
      Object.keys($scope.queries).forEach((e) => {
        if ($scope.queries[e]) {
          $scope.request.queries.push({
            fieldName: e,
            compareOperator: $scope.request.compareOperator,
    ...
    $scope.migrate = async function () {
      if ($routeParams.mixDbTableId) {
        $rootScope.isBusy = true;
        var result = await service.migrate($routeParams.mixDbTableId);
...
"use strict";
app.controller("MixDbDatabaseController", [
  "$scope",
  "$rootScope",
  "$location",
  "ngAppSettings",
  "$routeParams",
  "RestMixDbDatabaseService",
  function (
    $scope,
    $rootScope,
    $location,
    ...
    $scope.namingConventions = ["SnakeCase"];
    $scope.defaultAttr = null;
    $scope.databaseProvider = [
      "MySQL",
      "PostgreSQL",
      "SQLITE",
      "SQLSERVER",
...
"use strict";
app.factory("RestMixDbDatabaseService", [
  "BaseRestService",
  function (baseService) {
    var serviceFactory = Object.create(baseService);
    serviceFactory.init("mix-db-database");
...
"use strict";
app.controller("MixDbTableController", [
  "$scope",
  "$rootScope",
  "$location",
  "ngAppSettings",
  "$routeParams",
  "RestMixDbDataPortalService",
  "RestMixDbColumnPortalService",
  "RestMixDbTablePortalService",
  "RestMixDbDatabaseService",
  function (
    $scope,
    ...
    $scope.defaultAttr = null;
    $scope.actions = ["Delete"];
    $scope.viewmodelType = "mix-db-table";
    // $scope.request.selects = 'id,title,name,createdDateTime';
    $scope.orders = [
      { title: "Id", value: "Id" },
    ...
    $scope.request.searchColumns = "displayName,systemName";
    $scope.initList = async function () {
      $scope.request.mixDbDatabaseId = $routeParams.mixDbDatabaseId;
      await $scope.getList();
    };
    $scope.saveDatabase = function () {
      $scope.save($scope.viewmodel);
    ...
    $scope.getSingleSuccessCallback = async function () {
      if (!$scope.viewmodel.mixDbDatabaseId) {
        $scope.viewmodel.mixDbDatabaseId = $routeParams.mixDbDatabaseId;
      }
      $scope.viewmodel.namingConvention =
        $routeParams.namingConvention || "TitleCase";
      if ($scope.viewmodel.mixDbDatabaseId) {
        var resp = await mixDbDatabaseService.getSingle([
          $scope.viewmodel.mixDbDatabaseId,
        ]);
        if (resp.success) {
      ...
      if (!$scope.defaultAttr) {
        var getDefaultAttr = await mixDbColumnService.getDefault();
    ...
    $scope.migrate = async function () {
      if ($scope.viewmodel.id) {
        $rootScope.isBusy = true;
        var result = await mixDbTableService.migrate($scope.viewmodel);
        if (result.success) {
          $rootScope.showMessage(
    ...
    $scope.backup = async function () {
      if ($scope.viewmodel.id) {
        $rootScope.isBusy = true;
        var result = await mixDbTableService.backup($scope.viewmodel);
        if (result.success) {
          $rootScope.showMessage(
    ...
    $scope.restore = async function () {
      if ($scope.viewmodel.id) {
        $rootScope.isBusy = true;
        var result = await mixDbTableService.restore($scope.viewmodel);
        if (result.success) {
          $rootScope.showMessage(
    ...
    $scope.updateSchema = async function () {
      if ($scope.viewmodel.id) {
        $rootScope.isBusy = true;
        var result = await mixDbTableService.updateSchema($scope.viewmodel);
        if (result.success) {
          $rootScope.showMessage(
            "Please restart pool to apply new db schema",
            "warning"
...
app.controller("PageController", [
  ...
  function (
    ...
    $scope.request.culture = $rootScope.globalSettings.defaultCulture;
    var pageModuleService = $rootScope.getRestService("page-module");
    ...
    ($scope.additionalDatabaseRequest.searchColumns = "Type"),
      ($scope.additionalDatabaseRequest.compareOperator = "Equal"),
    ...
    $scope.getSingleSuccessCallback = function () {
      mixDbService.initDbName($scope.viewmodel.mixDbTableName);
      $scope.additionalDatabase = $rootScope.findObjectByKey(
        $scope.additionalDatabases,
        "systemName",
        $scope.viewmodel.mixDbTableName
      );
    ...
    $scope.loadAdditionalDatabases = async function () {
      ...
      if (getTypes.success) {
        ...
        if ($scope.request.mixDbTableName) {
          $scope.additionalDatabase = $rootScope.findObjectByKey(
            $scope.additionalDatabases,
            "mixDbTableName",
            $scope.request.mixDbTableName
          );
        }
        $scope.request.mixDbTableName = $routeParams.type || "";
        $scope.$apply();
      }
    };
    $scope.onSelectType = async function () {
      if (
        ...
        $scope.viewmodel.mixDbTableName = $scope.additionalDatabase.systemName;
        mixDbService.initDbName($scope.viewmodel.mixDbTableName);
        await $scope.loadAdditionalData();
      } else {
        $scope.viewmodel.mixDbTableName = null;
        $scope.additionalData = null;
      }
      $scope.request.mixDbTableName = $scope.additionalDatabase.systemName;
      $scope.createUrl = `/admin/page/create?type=${$scope.request.mixDbTableName}`;
      if ($routeParams.template) {
      ...
      if (
        ...
        await $scope.getDefault($scope.request.mixDbTableName);
      }
...
app.controller("PagePostController", [
  ...
  function (
    ...
    $scope.getList = async function (pageIndex) {
      ...
      $rootScope.isBusy = true;
      var id = $routeParams.id;
      $scope.request.query = "&parentId=" + id;
      var response = await service.getList($scope.request);
      $scope.canDrag =
        $scope.request.sortBy === "Priority" &&
        $scope.request.direction === "Asc";
...
app.controller("PostController", [
  ...
  function (
    ...
    ($scope.postTypeRequest.searchColumns = "Type"),
      ($scope.postTypeRequest.compareOperator = "Equal"),
    ...
    $scope.onSelectType = function () {
      ...
      $scope.request.additionalDatabase =
        $scope.additionalDatabase.mixDbTableName;
    ...
    $scope.saveSuccessCallback = async function () {
      if ($scope.additionalData && $scope.viewmodel.mixDbTableName) {
    ...
    $scope.getSingleSuccessCallback = async function () {
      if (!$scope.viewmodel.id) {
        $scope.viewmodel.mixDbTableName = $routeParams.type;
      }
      mixDbService.initDbName($scope.viewmodel.mixDbTableName);
      ...
      $scope.request.additionalDatabase = $scope.viewmodel.mixDbTableName;
    ...
    $scope.loadAdditionalData = async function () {
      if ($scope.viewmodel.mixDbTableName) {
        $scope.loadingData = true;
        mixDbService.initDbName($scope.viewmodel.mixDbTableName);
        if ($scope.viewmodel.id) {
          const getData = await mixDbService.getSingleByParent(
...
app.controller("RoleController", [
  ...
  function (
    ...
    $scope.initPermissions = async function () {
      ...
      $scope.createUrl = `/admin/mix-db-data/create?mixDbTableName=sysPermission&dataContentId=&parentId=${$scope.viewmodel.id}&parentName=Role&backUrl=${backUrl}`;
      $scope.updateUrl = "/admin/mix-db-data/details";
    };
      ...
      if ($routeParams.name) {
        ...
        if (resp && resp.success) {
          ...
          $scope.schedule = {
            ...
            groupName: $scope.trigger.group,
            repeatCount: $scope.trigger.repeatCount,
            interval: $scope.trigger.repeatInterval,
            intervalType: $scope.trigger.repeatIntervalUnit,
          };
...
app.controller("ServiceController", [
  ...
  function (
    ...
    $scope.saveSuccessCallback = function () {
      ...
      $scope.$apply();
      // if($scope.parentId){
      //     $location.url('/admin/mix-db-data/details?dataContentId='+ $scope.parentId);
      // }
      // else{
      //     $location.url('/admin/mix-db-data/list?mixDbTableId='+ $scope.viewmodel.mixDbTableId);
      // }
    };
    $scope.getList = async function () {
      $rootScope.isBusy = true;
    ...
    $scope.edit = function (data) {
      $scope.goToPath(
        "/admin/mix-db-data/details?dataContentId=" +
          data.id +
          "&mixDbTableId=" +
          $scope.mixDbTableId
      );
...
app.controller("ThemeImportController", [
  ...
  function (
    ...
    $scope.importThemeDto = {
      ...
      languageIds: [],
      mixDbDataIds: [],
      isIncludeAssets: false,
      isIncludeTemplates: false,
    };
    $scope.init = function () {};
    $scope.getSingleSuccessCallback = function () {
      $scope.assets = null;
...
"use strict";
app.controller("WorkflowTriggerController", [
  "$scope",
  "$rootScope",
  "ngAppSettings",
  "$location",
  "$routeParams",
  "WorkflowTriggerRestService",
  function (
    $scope,
    $rootScope,
    ngAppSettings,
    ...
    $scope.canDrag =
      $scope.request.sortBy !== "Priority" || $scope.request.direction !== "0";
    $scope.duration = function (milliseconds) {
      let result = "";
      const duration = moment.duration(milliseconds);
      if (duration.hours()) {
        result += `${Math.round(duration.hours())}h `;
      }
      if (duration.minutes()) {
        result += `${Math.round(duration.minutes())}m `;
      }
      if (duration.seconds()) {
        result += `${Math.round(duration.seconds())}s `;
      }
      if (duration.milliseconds()) {
        result += `${Math.round(duration.milliseconds())}ms `;
      }
      return result;
    };
  },
...
"use strict";
app.factory("WorkflowTriggerRestService", [
  "BaseRestService",
  function (baseService) {
    var serviceFactory = Object.create(baseService);
    serviceFactory.init("mix-workflow-trigger");
    return serviceFactory;
  },
]);
...
app.component("mixDatabaseEdm", {
  templateUrl:
    "/mix-app/views/app-portal/pages/mix-db-table/components/edm/view.html",
  controller: [
    "$rootScope",
    function ($rootScope) {
      var ctrl = this;
...
app.component("mixDbTables", {
  bindings: {
    model: "=",
  },
  templateUrl:
    "/mix-app/views/app-portal/pages/mix-db-database/components/mix-db-tables/view.html",
  controller: [
    "$rootScope",
    "$scope",
    "RestMixDbTablePortalService",
    function ($rootScope, $scope, tableService) {
      var ctrl = this;
      ctrl.request = $rootScope.getRequest();
      ctrl.$onInit = () => {
        ctrl.globalSettings = $rootScope.globalSettings;
        ctrl.isInRole = $rootScope.isInRole;
        ctrl.request.mixDbDatabaseId = ctrl.model.id;
        ctrl.loadTables();
      };
      ctrl.loadTables = async function (pageIndex) {
        if (pageIndex !== undefined) {
          ctrl.request.pageIndex = pageIndex;
        }
        if (ctrl.model.id) {
          var resp = await tableService.getList(ctrl.request);
          if (resp && resp.success) {
            ctrl.data = resp.data;
            $scope.$apply();
          }
      ...
      ctrl.remove = async function (tableId) {
        if (tableId) {
          if (
            confirm(
              "Deleted data will not able to recover, are you sure you want to delete this item?"
...
app.component("mixDatabaseMain", {
  templateUrl:
    "/mix-app/views/app-portal/pages/mix-db-table/components/main/view.html",
  controller: [
    "$rootScope",
    function ($rootScope) {
      var ctrl = this;
      ...
      ctrl.gennerateName = function () {
        if (
          ...
          if (ctrl.schema) {
            ctrl.model.systemName = `${ctrl.schema}${ctrl.model.systemName}`;
          }
        }
      };
    },
  ...
  controller: [
    ...
    function ($rootScope, $scope, ngAppSettings, service, postService) {
      ...
      ctrl.$onInit = async () => {
        ...
        if (ctrl.mixDbTableName) {
          ctrl.postRequest.mixDbTableName = ctrl.mixDbTableName;
        }
...
app.component("themeImportMixDbTables", {
  templateUrl:
    "/mix-app/views/app-portal/pages/theme-import/components/theme-import-mix-db-tables/view.html",
  bindings: {
    importData: "=",
    importThemeDto: "=",
  },
  controller: [
    ...
    function ($rootScope, $scope, ngAppSettings) {
      var ctrl = this;
      var service = $rootScope.getRestService("mix-db-table");
      ctrl.selectAllContent = false;
      ctrl.selectAllData = false;
      ctrl.request = angular.copy(ngAppSettings.request);
      ctrl.$onInit = async () => {
        ctrl.data = ctrl.importData.mixDbTables;
      };
      ...
      ctrl.selectContent = (mixDatabase, selected) => {
        ...
        ctrl.importThemeDto.mixDbTableIds = ctrl.updateArray(
          ctrl.importThemeDto.mixDbTableIds,
          [mixDatabase.id],
          selected
        );
        if (!selected) {
      ...
      ctrl.selectData = (mixDatabase, selected) => {
        ctrl.selectAllData = ctrl.selectAllData && selected;
        ctrl.importThemeDto.mixDbDataIds = ctrl.updateArray(
          ctrl.importThemeDto.mixDbDataIds,
          [mixDatabase.id],
          selected
        );
      };
      ...
      ctrl.checkExist = (mixDatabase) => {
        return ctrl.importData.existingDatabases.find(
          (m) => m.id == mixDatabase.id
        );
      };
      ctrl.unionArray = (a, b) => {
        return [...new Set([...a, ...b])];
      ...
      ctrl.colClass = (kind) => {
        switch (kind) {
          ...
          case "Changed":
            return "fa fa-pencil text-warning";
          case "Deleted":
...
app.component("themeExportMixDbTables", {
  templateUrl:
    "/mix-app/views/app-portal/pages/theme/components/theme-export-mix-db-tables/view.html",
  controller: [
    "$rootScope",
    "$scope",
    "ngAppSettings",
    ...
    function ($rootScope, $scope, ngAppSettings, dbContextService) {
      ...
      ctrl.selectContent = (mixDatabaseContext, selected) => {
        ctrl.selectAllContent = ctrl.selectAllContent && selected;
        ctrl.exportThemeDto.content.mixDbDatabaseIds = ctrl.updateArray(
          ctrl.exportThemeDto.content.mixDbDatabaseIds,
      ...
      ctrl.selectDbContent = (mixDatabase, selected) => {
        ...
        ctrl.exportThemeDto.content.mixDbTableIds = ctrl.updateArray(
          ctrl.exportThemeDto.content.mixDbTableIds,
          [mixDatabase.id],
          selected
        );
        if (!selected) {
      ...
      ctrl.selectData = (mixDatabase, selected) => {
        ctrl.selectAllData = ctrl.selectAllData && selected;
        ctrl.exportThemeDto.associations.mixDbTableIds = ctrl.updateArray(
          ctrl.exportThemeDto.associations.mixDbTableIds,
          [mixDatabase.id],
          selected
        );
      };
...
modules.component("additionalColumn", {
  ...
  controller: [
    ...
    function ($rootScope, $scope, columnService, dataService) {
      ...
      ctrl.column = {
        dataType: "Text",
        mixDbTableName: "sysAdditionalColumn",
        mixDbTableId: 6,
        status: "Published",
      };
        ...
        if (!ctrl.additionalData) {
          if (!ctrl.additionalDataId) {
            const obj = {
              ...
              parentId: ctrl.parentId,
              databaseName: ctrl.mixDbTableName,
            };
            const getData = await dataService.getAdditionalData(obj);
            if (getData.success) {
              ctrl.additionalData = getData.data;
              ctrl.additionalData.mixDbTableName = ctrl.mixDbTableName;
        ...
        if (file !== null) {
          ...
          reader.onload = function () {
            ...
            if (ctrl.postedFile) {
              ...
              ctrl.postedFile.fileBase64 = reader.result;
            }
...
modules.component("filterList", {
  ...
  controller: [
    ...
    function ($scope, $rootScope, ngAppSettings, cultureService) {
      ...
      ctrl.conjunctions = ["And", "Or"];
      ctrl.compareOperators = ["Equal", "Like", "ILike", "NotEqual"];
      ctrl.init = async function () {
        ...
        ctrl.request.sortBy = ctrl.request.sortBy || ctrl.arrOrderBy[0];
...
modules.component("listMixColumn", {
  ...
  controller: [
    ...
    function (
      ...
      ctrl.$onInit = async function () {
        ctrl.dataTypes = $rootScope.globalSettings.dataTypes;
        ctrl.request.mixDbDatabaseId = ctrl.mixDbDatabaseId;
        ctrl.updateNamingConvention();
      ...
      ctrl.addColumn = function (
        ...
        if (ctrl.columns) {
          ...
          t.columnConfigurations.isRequire = required;
          t.defaultValue = defaultValue;
          if (colName) {
            t.displayName = colName;
            t.systemName = colName;
          }
          if (dataType) {
            t.dataType = dataType;
          }
      ...
      ctrl.addDefaultColumns = () => {
        ctrl.addColumn(ctrl.fieldNames.id, "Integer", true);
        ctrl.addColumn(ctrl.fieldNames.createdBy, "String");
        ctrl.addColumn(ctrl.fieldNames.modifiedByBy, "String");
        ctrl.addColumn(ctrl.fieldNames.createdDateTime, "DateTime", true);
        ctrl.addColumn(ctrl.fieldNames.lastModified, "DateTime", true);
        ctrl.addColumn(ctrl.fieldNames.priority, "Integer", false, 0);
      };
      ctrl.showModal = (id) => {
      ...
      ctrl.updateNamingConvention = () => {
        ctrl.fieldNames = {
          id: ctrl.namingConvention == "TitleCase" ? "Id" : "id",
          parentId:
            ctrl.namingConvention == "TitleCase" ? "ParentId" : "parent_id",
          parentDatabaseName:
            ctrl.namingConvention == "TitleCase"
              ? "ParentDatabaseName"
              : "parent_database_name",
          childId:
            ctrl.namingConvention == "TitleCase" ? "ChildId" : "child_id",
          childDatabaseName:
            ctrl.namingConvention == "TitleCase"
              ? "ChildDatabaseName"
              : "child_database_name",
          ...
          lastModified:
            ctrl.namingConvention == "TitleCase"
              ...
              : "last_modified",
          tenantId:
            ctrl.namingConvention == "TitleCase" ? "TenantId" : "mix_tenant_id",
          createdBy:
            ctrl.namingConvention == "TitleCase" ? "CreatedBy" : "created_by",
          modifiedBy:
            ctrl.namingConvention == "TitleCase" ? "ModifiedBy" : "modified_by",
          priority:
            ctrl.namingConvention == "TitleCase" ? "Priority" : "priority",
          status: ctrl.namingConvention == "TitleCase" ? "Status" : "status",
          isDeleted:
            ctrl.namingConvention == "TitleCase" ? "IsDeleted" : "is_deleted",
        };
      };
      ...
      ctrl.uploadFile = async function (file) {
        if (file !== null) {
          ...
          if (file) {
            ...
            if (response.success) {
              ctrl.media = response.data.url;
              $rootScope.isBusy = false;
              ctrl.srcUrl = response.data.url;
              $scope.$apply();
            } else {
              $rootScope.showErrors(response.errors);
              $rootScope.isBusy = false;
  ...
  controller: [
    ...
    function (
      ...
      ctrl.loadDatabase = async function () {
        var getMixDatabase = await databaseService.getByName(["mix_metadata"]);
...
modules.component("mixDbDataValueEditor", {
  templateUrl:
    "/mix-app/views/app-portal/components/mix-db-data-value-editor/view.html",
  bindings: {
    mixDbDataValue: "=?",
    parentType: "=?",
    parentId: "=?",
  ...
  controller: [
    ...
    function (
      ...
      ctrl.previousId = null;
      ctrl.$doCheck = function () {
        if (ctrl.mixDbDataValue && ctrl.previousId !== ctrl.mixDbDataValue.id) {
          ctrl.previousId = ctrl.mixDbDataValue.id;
          ctrl.initData();
        }
      }.bind(ctrl);
      ctrl.$onInit = function () {
      ...
      ctrl.initData = async function () {
        setTimeout(() => {
          if (!ctrl.mixDbDataValue.id) {
            ctrl.initDefaultValue();
          }
          switch (ctrl.mixDbDataValue.dataType.toLowerCase()) {
            case "datetime":
            case "date":
            case "time":
              if (ctrl.mixDbDataValue.dateTimeValue) {
                ctrl.mixDbDataValue.dateObj = new Date(
                  ctrl.mixDbDataValue.dateTimeValue
                );
                $scope.$apply();
              }
              break;
            case "reference": // reference
              if (ctrl.mixDbDataValue.column.referenceId && ctrl.parentId) {
                ctrl.mixDbDataValue.integerValue =
                  ctrl.mixDbDataValue.column.referenceId;
                // navService.getSingle(["default"]).then((resp) => {
                //   ctrl.defaultDataModel = resp;
                //   ctrl.defaultDataModel.mixDbTableId =
                //     ctrl.mixDbDataValue.column.referenceId;
            ...
            default:
              if (
                ctrl.mixDbDataValue.column &&
                ctrl.mixDbDataValue.column.isEncrypt &&
                ctrl.mixDbDataValue.encryptValue
              ) {
                var encryptedData = {
                  key: ctrl.mixDbDataValue.encryptKey,
                  data: ctrl.mixDbDataValue.encryptValue,
                };
                ctrl.mixDbDataValue.stringValue =
              ...
              if (
                ...
                ctrl.mixDbDataValue.stringValue =
                  ctrl.mixDbDataValue.column.defaultValue;
                $scope.$apply();
              }
              break;
          }
      ...
      ctrl.initDefaultValue = async function () {
        switch (ctrl.mixDbDataValue.dataType) {
          case "datetime":
          case "date":
          case "time":
            if (ctrl.mixDbDataValue.column.defaultValue) {
              ctrl.mixDbDataValue.dateObj = new Date(
                ctrl.mixDbDataValue.column.defaultValue
              );
              ctrl.mixDbDataValue.stringValue =
          ...
          case "double":
            if (ctrl.mixDbDataValue.column.defaultValue) {
              ctrl.mixDbDataValue.doubleValue = parseFloat(
                ctrl.mixDbDataValue.column.defaultValue
              );
              ctrl.mixDbDataValue.stringValue =
                ctrl.mixDbDataValue.column.defaultValue;
          ...
          case "boolean":
            if (ctrl.mixDbDataValue.column.defaultValue) {
              ctrl.mixDbDataValue.booleanValue =
                ctrl.mixDbDataValue.column.defaultValue == "true";
              ctrl.mixDbDataValue.stringValue =
                ctrl.mixDbDataValue.column.defaultValue;
            }
            break;
      ...
      ctrl.updateStringValue = async function (dataType) {
        switch (dataType.toLowerCase()) {
          ...
          case "date":
          case "time":
            if (ctrl.mixDbDataValue.dateObj) {
              ctrl.mixDbDataValue.dateTimeValue =
                ctrl.mixDbDataValue.dateObj.toISOString();
              ctrl.mixDbDataValue.stringValue =
                ctrl.mixDbDataValue.dateTimeValue;
            }
            break;
          case "double":
            if (ctrl.mixDbDataValue.doubleValue) {
              ctrl.mixDbDataValue.stringValue =
                ctrl.mixDbDataValue.doubleValue.toString();
            }
            break;
          case "boolean":
            if (ctrl.mixDbDataValue.booleanValue != null) {
              ctrl.mixDbDataValue.stringValue =
                ctrl.mixDbDataValue.booleanValue.toString();
            }
            break;
      ...
      ctrl.updateRefData = function (nav) {
        ctrl.goToPath(`/admin/mix-db-data/details?dataContentId=${nav.data.id}
                &mixDbTableId=${nav.data.mixDbTableId}
                &parentId=${ctrl.parentId}
                &parentType=${ctrl.parentType}`);
        // ctrl.refDataModel = nav;
        // var e = $(".pane-form-" + ctrl.mixDbDataValue.column.referenceId)[0];
        // angular.element(e).triggerHandler('click');
        // $location.url('/admin/mix-db-data/details?dataContentId='+ item.id +'&mixDbTableId=' + item.mixDbTableId+'&parentType=' + item.parentType+'&parentId=' + item.parentId);
      };
...
modules.component("mixDatabaseNavData", {
  templateUrl:
    "/mix-app/views/app-portal/components/mix-db-data-nav-data/view.html",
  bindings: {
    nav: "=",
    parentId: "=",
    parentType: "=",
      ...
      ctrl.saveData = function (data) {
        ...
        dataService.save(data).then((resp) => {
          if (resp.success) {
            ...
            ctrl.selected.mixDbTableId = resp.data.mixDbTableId;
            ctrl.selected.mixDbTableName = resp.data.mixDbTableName;
...
modules.component("mixDbDataValues", {
  templateUrl:
    "/mix-app/views/app-portal/components/mix-db-data-values/view.html",
  bindings: {
    database: "=?",
    request: "=",
    mixDbTableName: "=?",
  ...
  controller: [
    ...
    function (
      ...
      ctrl.conjunctions = ["Or", "And"];
      ctrl.selectedProp = null;
      ctrl.mixConfigurations = $rootScope.globalSettings;

      ctrl.$onInit = async function () {
        dataService.initDbName(ctrl.mixDbTableName);
        ...
        ctrl.request.name = ctrl.mixDbTableName;
        ctrl.request.sortBy = ctrl.fieldNames.id;
        ...
        ctrl.request.conjunction = ctrl.conjunction;
        if (!ctrl.data) {
          await ctrl.loadData();
        }
        ctrl.createUrl = `/admin/mix-db-data/create?mixDbTableId=${
          ctrl.database.id
        }&mixDbTableName=${ctrl.database.systemName}&mixDatabaseTitle=${
          ctrl.mixDatabaseTitle
        }&dataContentId=&relationship=${ctrl.relationship || ""}&parentId=${
          ctrl.parentId || ctrl.guidParentId || ""
        }&parentName=${ctrl.parentName || ""}`;
      };
      ctrl.toCamelCase = (txt) => {
        return txt.charAt(0).toLowerCase() + txt.substr(1);
      };
      ctrl.loadData = async function (pageIndex, pagingState = null) {
        $rootScope.isBusy = true;
        if (pageIndex !== undefined) {
          ctrl.request.pageIndex = pageIndex;
        }
        if (pagingState) {
          ctrl.request.pagingState = pagingState;
        }
        dataService.initDbName(ctrl.request.mixDbTableName);
          ...
          Object.keys(ctrl.queries).forEach((e) => {
            if (ctrl.queries[e]) {
              ctrl.request.queries.push({
                fieldName: e,
                compareOperator: ctrl.request.compareOperator,
      ...
      ctrl.filter = function (pageIndex) {
        ctrl.loadData(pageIndex);
      };
      ...
      ctrl.update = function (data) {
        let url = `/admin/mix-db-data/details?dataContentId=${
          data.id
        }&mixDbTableName=${ctrl.request.mixDbTableName}&mixDbTableTitle=${
          ctrl.mixDatabaseTitle
      ...
      ctrl.view = function (item) {
        ...
        $rootScope.preview("mix-db-data", obj, null, "modal-lg");
      };
    },
  ],
});
...
modules.component("mixDatabaseNavValues", {
  templateUrl:
    "/mix-app/views/app-portal/components/mix-db-data-nav-values/view.html",
  bindings: {
    mixDbTableId: "=",
    mixDbTableName: "=",
    guidParentId: "=",
  ...
  controller: [
    ...
    function ($rootScope, $scope, ngAppSettings, navService, columnService) {
      ...
      ctrl.$onInit = async function () {
        ...
        if (!ctrl.createUrl) {
          ctrl.createUrl = "/admin/mix-db-data/create";
        }
        if (!ctrl.updateUrl) {
          ctrl.updateUrl = "/admin/mix-db-data/details";
        }
        if (!ctrl.columns) {
          var getFields = await columnService.initData(
            ctrl.mixDbTableName || ctrl.mixDbTableId
          );
      ...
      ctrl.loadData = function () {
        ctrl.request.mixDbTableId = ctrl.mixDbTableId || 0;
        ctrl.request.mixDbTableName = ctrl.mixDbTableName || null;
        ctrl.request.guidParentId = ctrl.guidParentId;
      ...
      ctrl.updateData = function (nav) {
        $rootScope.goToPath(
          `${ctrl.updateUrl}?dataContentId=${nav.childDataContent.id}&mixDbTableId=${nav.mixDbTableId}&guidParentId=${ctrl.guidParentId}&parentType=${ctrl.parentType}`
        );
        // ctrl.refDataModel = nav;
        // var e = $(".pane-form-" + ctrl.mixDbDataValue.column.referenceId)[0];
        // angular.element(e).triggerHandler('click');
          ...
          if (resp.success) {
            ...
            navService.save("portal", ctrl.refDataModel).then((resp) => {
              if (resp.success) {
                ...
                ctrl.refDataModel = angular.copy(ctrl.defaultDataModel);
                var e = $(
                  ".pane-data-" + ctrl.mixDbDataValue.column.referenceId
                )[0];
                angular.element(e).triggerHandler("click");
                $rootScope.isBusy = false;
                $scope.$apply();
...
modules.component("mixDatabaseNavs", {
  templateUrl:
    "/mix-app/views/app-portal/components/mix-db-table-navs/view.html",
  bindings: {
    parentId: "=",
    parentType: "=",
    mixDatabaseNavs: "=?",
...
modules.component("mixDatabaseOneToMany", {
  templateUrl:
    "/mix-app/views/app-portal/components/mix-db-table-one-to-many/view.html",
  bindings: {
    database: "=?",
    fieldNames: "=?",
    mixDbTableName: "=?",
  ...
  controller: [
    ...
    function (
      ...
      ctrl.$onInit = async function () {
        ...
        }&dataContentId=&relationship=${
          ctrl.relationship || ""
        }&${ctrl.getParentIdColName()}=${
          ctrl.parentId || ctrl.guidParentId || ""
        }&parentName=${ctrl.parentName || ""}`;
        $scope.$apply();
      };
      ctrl.getParentIdColName = () => {
        return `${ctrl.parentName}_id`;
      };
      ctrl.toCamelCase = (txt) => {
        return txt.charAt(0).toLowerCase() + txt.substr(1);
      };
      ...
      ctrl.update = function (item) {
        let url = `/admin/mix-db-data/details?dataContentId=${
          item.id
        }&mixDbTableName=${ctrl.mixDbTableName}&mixDatabaseTitle=${
          ctrl.mixDatabaseTitle
        }&guidParentId=${ctrl.guidParentId || ""}&parentId=${
          ctrl.parentId || ""
        }&parentName=${ctrl.parentName || ""}`;
...
modules.component("mixDbDataFilter", {
  templateUrl:
    "/mix-app/views/app-portal/components/mix-db-data-filter/view.html",
  bindings: {
    query: "=",
    mixDbTableName: "=?",
    mixDbTableId: "=?",
  ...
  controller: [
    ...
    function ($rootScope, $scope, ngAppSettings, dataService) {
      ...
      ctrl.loadData = async function (pageIndex) {
        ...
        if (ctrl.mixDbTableId) {
          ctrl.request.mixDbTableId = ctrl.mixDbTableId;
        }
        if (ctrl.mixDbTableName) {
          ctrl.request.mixDbTableName = ctrl.mixDbTableName;
        }
        if (ctrl.filterType) {
...
modules.component("mixDbDataForm", {
  templateUrl:
    "/mix-app/views/app-portal/components/mix-db-data-form/view.html",
  bindings: {
    database: "=?",
    mixDbTableId: "=?",
    mixDbTableName: "=?",
  ...
  controller: [
    ...
    function (
      ...
      ctrl.init = async function () {
        ...
        await service.initDbName(ctrl.mixDbTableName);
        await ctrl.loadData();
        ctrl.connectHub();
        ctrl.isBusy = false;
        $scope.$apply();
      ...
      ctrl.joinRooms = function () {
        ctrl.connection.invoke("JoinRooms", [ctrl.mixDbTableName]);
      };
      ctrl.hubCreateData = function () {
        let msg = {
          connectionId: ctrl.hubRequest.from.connectionId,
          mixDbName: ctrl.mixDbTableName,
          requestedBy: "",
      ...
      ctrl.loadData = async function () {
        ...
        */
        ...
        //       childDatabaseName: ctrl.mixDbTableName,
        //     };
        //   }
      ...
      ctrl.loadExistingData = async function () {
        service.initDbName(ctrl.mixDbTableName);
        ...
        if (ctrl.mixDataContent) {
          ...
          ctrl.backUrl =
            ctrl.backUrl ??
            `/admin/mix-db-data/list?mixDbTableId=${ctrl.mixDataContent.mixDbTableId}&mixDbTableName=${ctrl.mixDbTableName}&mixDatabaseTitle=${ctrl.mixDatabaseTitle}`;
          // await ctrl.loadDefaultModel();
          ctrl.isBusy = false;
        } else {
          if (getData) {
      ...
      ctrl.loadSelected = function () {
        if (ctrl.selectedList.data.length) {
          ...
          ctrl.mixDataContent.mixDbTableId = ctrl.mixDbTableId;
          ctrl.mixDataContent.mixDbTableName = ctrl.mixDbTableName;
          if (ctrl.parentId) {
            ctrl.mixDataContent[ctrl.fieldNames.parentId] = ctrl.parentId;
          }
...
modules.component("mixDatabaseNestedDataValues", {
  templateUrl:
    "/mix-app/views/app-portal/components/mix-db-data-nested-data-values/view.html",
  bindings: {
    database: "=?",
    fieldNames: "=?",
    mixDbTableName: "=?",
  ...
  controller: [
    ...
    function (
      ...
      ctrl.removeConfirmed = async function (dataContentId) {
        $rootScope.isBusy = true;
        dataService.initDbName(ctrl.mixDbTableName);
        var result = await dataService.deleteRelationship([dataContentId]);
        if (result.success) {
          if (ctrl.onDelete) {
            ctrl.onDelete({ data: dataContentId });
          }
        ...
        if (file !== null) {
          ...
          reader.onload = function () {
            if (ctrl.mediaFile) {
              ...
              ctrl.mediaFile.fileBase64 = reader.result;
            }
            $rootScope.isBusy = false;
            $scope.$apply();
          };
...
modules.component("mixDatabaseColumn", {
  templateUrl:
    "/mix-app/views/app-portal/components/mix-db-table-column/view.html",
  bindings: {
    column: "=",
  },
  controller: [
...
app.controller("MixNavigationController", [
  ...
  function (
    ...
    $scope.uploadMedia = async function () {
      ...
      if (resp && resp.success) {
        $scope.activedMedia = resp.data.url;
        $scope.getList();
        $scope.$apply();
      } else {
        if (resp) {
    ...
    $scope.getBase64 = function (file) {
      if (file !== null && $scope.postedFile) {
        ...
        reader.onload = function () {
          ...
          $scope.activedMedia.mediaFile.fileBase64 = reader.result;
          $rootScope.isBusy = false;
          $scope.$apply();
        };
        reader.onerror = function (error) {
      ...
      ctrl.initData = async function () {
        setTimeout(() => {
          switch (ctrl.type) {
            ...
            case "reference": // reference
              if (ctrl.referenceId) {
                ctrl.refRequest.mixDbTableId = ctrl.referenceId;
  ...
  controller: [
    ...
    function (
      ...
      ctrl.loadHelperUrl = function () {
        ...
        if (portalUrl.startsWith("/admin/mix-db-data/list?mixDbTableId=2")) {
          ctrl.trustedUrl = $sce.trustAsResourceUrl(
        ...
        if (portalUrl.startsWith("/admin/mix-db-table")) {
          ctrl.trustedUrl = $sce.trustAsResourceUrl(
            defaultUrl + "/basic-usage-database"
          );
        }
        if (portalUrl.startsWith("/admin/mix-db-data/list?mixDbTableId=7")) {
          ctrl.trustedUrl = $sce.trustAsResourceUrl(
            defaultUrl + "/basic-usage-category"
          );
        }
        if (portalUrl.startsWith("/admin/mix-db-data/list?mixDbTableId=8")) {
          ctrl.trustedUrl = $sce.trustAsResourceUrl(
  ...
  controller: [
    ...
    function (
      ...
      ctrl.showDbModel = function () {
        $(`#modal-nav-data-${ctrl.mixDbTableName}`).modal("show");
      };
  ...
  controller: [
    ...
    function (
      ...
      ctrl.loadSelected = async function () {
        ctrl.navRequest = angular.copy(ngAppSettings.request);
        ctrl.navRequest.mixDbTableId = ctrl.mixDbTableId;
        ctrl.navRequest.mixDbTableName = ctrl.mixDbTableName;
      ...
      ctrl.loadDefaultModel = async function () {
        ...
        ctrl.defaultNav = {
          ...
          parentType: ctrl.parentType,
          mixDbTableId: ctrl.mixDbTableId,
          mixDbTableName: ctrl.mixDbTableName,
          status: "Published",
          childDataContent: null,
        };
        if (!ctrl.columns) {
          var getMixDatbase = ctrl.mixDbTableId
            ? await databaseService.getSingle([ctrl.mixDbTableId])
            : await databaseService.getByName([ctrl.mixDbTableName]);
          if (getMixDatbase.success) {
            ctrl.columns = getMixDatbase.data.columns;
            ctrl.mixDbTableId = getMixDatbase.data.id;
            ctrl.mixDbTableName = getMixDatbase.data.systemName;
            ctrl.defaultNav.mixDbTableId = getMixDatbase.data.id;
            ctrl.defaultNav.mixDbTableName = getMixDatbase.data.systemName;
            $scope.$apply();
          }
        }
      };
      ctrl.loadDefaultData = async function () {
        ...
        if (ctrl.defaultData) {
          ctrl.defaultData.mixDbTableId = ctrl.mixDbTableId || 0;
          ctrl.defaultData.mixDbTableName = ctrl.mixDbTableName;
        }
        if (!ctrl.mixDbData) {
          ctrl.mixDbData = angular.copy(ctrl.defaultData);
        }
        if (!ctrl.mixDbTableId) {
          ctrl.mixDbTableId = ctrl.defaultData.mixDbTableId;
        }
        if (!ctrl.mixDbTableName) {
          ctrl.mixDbTableName = ctrl.defaultData.mixDbTableName;
        }
      };
      ...
      ctrl.createData = function () {
        if (ctrl.newTitle) {
          ...
          if (!tmp) {
            ctrl.isBusy = true;
            ctrl.mixDbData.intParentId = 0;
            ctrl.mixDbData.parentType = "Set";
            ctrl.mixDbData.data.title = ctrl.newTitle;
            ctrl.mixDbData.data.slug = $rootScope.generateKeyword(
              ctrl.newTitle,
              "-"
            );
            ctrl.mixDbData.data.type = ctrl.type;
            dataService.save(ctrl.mixDbData).then((resp) => {
              if (resp.success) {
                ctrl.mixDbData.id = resp.data;
                ctrl.data.items.push(ctrl.mixDbData);
                ctrl.reload();
  ...
  controller: [
    ...
    function ($rootScope, $scope, ngAppSettings, $location) {
      ...
      ctrl.$onInit = async () => {
        ...
        ctrl.associationRequest.sortBy = "id";
        ctrl.associationRequest.direction = "Desc";
...
modules.component("serviceHubPortal", {
  ...
  controller: [
    ...
    function ($rootScope, $scope, columnService, service, userServices) {
      ...
      ctrl.init = function () {
        ctrl.mixDbTableId = ctrl.mixDbTableId || 0;
        ctrl.request.specificulture = service.lang;
        ctrl.request.room = ctrl.mixDbTableName;
        ctrl.request.isSave = ctrl.isSave == "true" || false;
        ctrl.startConnection("serviceHub", ctrl.checkLoginStatus);
      };
      ctrl.loadData = async function () {
        /*
                    If input is data id => load ctrl.mixDbData from service and handle it independently
                    Else modify input ctrl.mixDbData
                */
        $rootScope.isBusy = true;
        var getDefault = await service.initData(ctrl.mixDbTableName);
        ...
        var getFields = await columnService.initData(ctrl.mixDbTableName);
        if (getFields.success) {
          ctrl.columns = getFields.data;
        }
      };
      ctrl.submit = async function () {
        if (ctrl.validate()) {
          ...
          ctrl.request.data = ctrl.mixDbData.data;
      ...
      ctrl.validate = function () {
        ...
        angular.forEach(ctrl.columns, function (column) {
          if (column.regex) {
            var regex = RegExp(column.regex, "g");
            isValid = regex.test(ctrl.mixDbData.data[column.name]);
            if (!isValid) {
              ctrl.errors.push(`${column.name} is not match Regex`);
            }
          }
          if (isValid && column.isEncrypt) {
            ctrl.mixDbData.data[column.name] = $rootScope.encrypt(
              ctrl.mixDbData.data[column.name]
            );
          }
        });
        return isValid;
    ...
    function ($rootScope, $scope, service, viewmodel, connectionManager) {
      ...
      var _hub,
        ...
        _getUsername = function () {
          _startSession("User " + Math.floor(Math.random() * 10000 + 1));
        },
