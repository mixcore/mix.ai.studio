<script lang="ts">
  ...
  function selectFile(file: FileItem, path: string) {
    if (file.content) {
...
<ul>
  {#each Object.entries(files) as [name, file] (name)}
    <li>
      <button class="group flex cursor-pointer items-center px-2 py-1 text-sm hover:bg-primary/25 w-full text-left" on:click={() => selectFile(file as FileItem, name)}>
        <div class="mr-1 flex-shrink-0" style="margin-left: {level * 16}px;">
          {#if !(file as FileItem).content}
            <svg xmlns="http://www.w3.org/2000/svg" width="100%" height="100%" viewBox="0 -960 960 960" class="shrink-0 h-5 w-5 transition-transform duration-200 {collapsed.has(name) ? '-rotate-90' : 'rotate-0'}" fill="currentColor"><path d="M530-481 353-658q-9-9-8.5-21t9.5-21 21.5-9 21.5 9l198 198q5 5 7 10t2 11-2 11-7 10L396-261q-9 9-21 8.5t-21-9.5-9-21.5 9-21.5z"></path></svg>
          {:else}
      ...
      {#if !(file as FileItem).content && !collapsed.has(name)}
