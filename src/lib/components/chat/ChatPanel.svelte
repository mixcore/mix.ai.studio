<script lang="ts">
  ...
  const baseUrl =
    import.meta.env.VITE_MIXCORE_PREVIEW_ENDPOINT || "https://mixcore.net";
    ...
    if (currentMode === "mixcore") {
      try {
        chatService = new ChatService({
          ...
          accessTokenFactory: () =>
            localStorage.getItem("mixcore_access_token") || "",
  ...
  $: if (
    chatContainer &&
    ($chatMessages.length > previousMessageCount || $chatStreamingMessage)
  ) {
...
<div class="flex flex-col h-full bg-base-100">
  ...
  <div class="p-4 border-b border-base-300">
    <div class="flex items-center justify-between">
      ...
      <div class="flex items-center gap-2">
        <button
          class="btn btn-xs {currentMode === 'mixcore'
            ? 'btn-primary'
            : 'btn-outline'}"
        ...
        <button
          class="btn btn-xs {currentMode === 'external'
            ? 'btn-primary'
            : 'btn-outline'}"
    ...
    {#if currentMode === "external"}
      <div class="mt-2 flex items-center gap-2">
        ...
        >
          {#each availableModels.filter((m) => {
            ...
            const providerKey = m.provider === "OpenAI" ? "openai" : m.provider === "Anthropic" ? "claude" : m.provider === "Google" ? "gemini" : m.provider === "DeepSeek" ? "deepseek" : m.provider.toLowerCase();
      ...
      <div class="mt-2 flex items-center gap-2">
        ...
        <div
          class="badge badge-xs {$mcpTools.length > 0
            ? 'badge-success'
            : 'badge-neutral'}"
        >
          {$mcpTools.length} tools from {$mcpConnections.filter(
            (c) => c.connected
          ).length} servers
        </div>
        {#if $mcpTools.length > 0}
          <div class="dropdown dropdown-end">
            <div
              tabindex="0"
              role="button"
              class="btn btn-xs btn-ghost text-xs text-base-content/60 hover:text-base-content"
            >
              View Tools
            </div>
            <div
              ...
              <div class="grid grid-cols-2 gap-1">
                {#each $mcpTools as { tool }}
                  <div class="tooltip tooltip-top" data-tip={tool.description}>
                    <div
                      class="badge badge-xs badge-outline text-xs p-2 cursor-help"
                    >
                      {tool.name}
                    </div>
                  </div>
    ...
    {#if $chatMessages.length === 0}
      <div class="flex flex-col items-center justify-center h-full text-center">
        ...
        <p class="text-sm text-base-content/60 max-w-sm">
          {#if currentMode === "mixcore"}
            Using Mixcore Streaming API - Describe what you want to build and
            I'll help you create it step by step.
          {:else}
            Using {$selectedModel.name} - Describe what you want to build and I'll
            help you create it step by step.
          {/if}
        </p>
      </div>
    {:else}
